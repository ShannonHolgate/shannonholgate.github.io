...
A Block is an "inline anonymous collection of code" - You could call it an inline function you can assign to a variable.

<pre><code class="language-clike">int (^addition)(int,int) = 
		^(int num1, int num2) {return num1 + num2};</code></pre>

The developer API states a block to have the following properties:

* A typed argument list like a function
* Inferred (x+1 could be int) or declared return type
* Can capture state from the lexical scope where it's defined (function where it's defined)
...
So now that we’ve declared the block above, lets put it to use: 
...
We can use blocks as callbacks from functions, meaning we can drop a block in as a parameter in a function header then call that block when we want.
...
looking closely at the final parameter you can see the block header being delcared, just like before:
...
So nothing special has happened yet, just printing a couple of parameters then calling the block we’ve declared in the header.
Lets move onto implementation where blocks can shine.
...
You can manipulate your variables anyway you want within the block, the beauty being it’s functionality is called at the precise moment that it’s referenced in the underlying method.
...
### Why use Blocks?
Use your imagination, the power of blocks allows incredible things to happen with simplicity. I find them very useful when I want to use a method which has the potential to fail in several ways.
...
We will declare a method in here to get some messages from a currentUser object. The method should through a simple error through a block if no user is logged in.
...
This has been a short introduction to Objective-C blocks, of course they can get incredibly complicated very quickly if you desire but I’m sure you can use this to build on that knowledge. Feel free to drop me a line if you have any questions!
